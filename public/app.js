const App = {
    ...
    showRewardedAd() {
        ...
        contentArea.innerHTML = `
            <video id="rewardedVideo" style="width:100%;height:100%;object-fit:cover;border-radius:12px;" muted playsinline></video>
            <div class="ad-click-overlay" id="adClickOverlay" style="position:absolute;inset:0;cursor:pointer;z-index:3;"></div>
        `;
    ...
    loadVastAd() {
        const videoEl = document.getElementById('rewardedVideo');
        const overlay = document.getElementById('adClickOverlay');
        if (!videoEl) return;

        const vastUrl = 'https://s.magsrv.com/v1/vast.php?idz=5972144';
        const proxyUrl = this.apiBase() + '/api/proxy?url=' + encodeURIComponent(vastUrl);
        ...
        fetch(proxyUrl)
            ...
            .then(xmlText => {
                ...
                if (mediaFile && mediaFile.textContent.trim()) {
                    ...
                    if (clickThrough && overlay) {
                        overlay.addEventListener('click', () => {
                            window.open(clickThrough.textContent.trim(), '_blank');
                        });
                    }
    ...
    showFallbackAd(videoEl) {
        ...
        fallback.href = ad.url;
        fallback.target = '_blank';
        fallback.rel = 'noopener';
        fallback.innerHTML = `<img src="${ad.img}" alt="${ad.label}" style="width:100%;height:100%;object-fit:cover;border-radius:12px;">`;
